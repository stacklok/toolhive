...
impl StdioTransport {
    ...
    async fn handle_sse_connection(&self, req: Request<Body>) -> Result<Response<Body>> {
        ...
        let (tx, mut rx) = mpsc::channel::<String>(100);
        {
            ...
            log::debug!(
                "Registered SSE client. Active sessions: {:?}",
        ...
        let messages_to_send = {
            ...
            if !pending.is_empty() {
                log::debug!(
                    "Processing {} pending messages for new client",
                    pending.len()
                );
                ...
                *pending = remaining_messages;
                ...
                log::debug!(
                    "Found {} messages for client {}",
                    messages_for_client.len(),
                    client_id
                );
                messages_for_client
            } else {
    ...
    async fn handle_post_request(&self, req: Request<Body>) -> Result<Response<Body>> {
        ...
        let session_id = query.split('&').find_map(|param| {
        ...
        let client_exists = {
            let clients = self.sse_clients.lock().await;
            log::debug!(
                "Checking if session exists. Active sessions: {:?}",
                clients.keys().map(|k| k.to_string()).collect::<Vec<_>>()
            );
    ...
    async fn start_http_server(&self, port: u16) -> Result<()> {
        ...
        log::debug!(
            "HTTP with SSE transport started for STDIO container {} on port {}",
            container_name,
            port
        );
    ...
    async fn process_stdout_chunk(
        ...
        if start_idx < buffer.len() {
            ...
            log::debug!(
                "Keeping partial line in buffer: {:?}",
    ...
    fn log_json_rpc_message(json_rpc: &JsonRpcMessage) {
        ...
        if json_rpc.is_request() {
            ...
            log::debug!(
                "Received JSON-RPC response: id={:?}, result={:?}, error={:?}",
                json_rpc.id,
            ...
            log::debug!(
                "Received JSON-RPC notification: method={}",
                json_rpc.method.as_ref().unwrap_or(&"none".to_string())
            );
        } else {
    ...
    pub async fn handle_request(&self, req: Request<Body>) -> Result<Response<Body>> {
        ...
        if let Some(mut rx) = rx_option.take() {
            if let Some(json_rpc) = rx.recv().await {
                ...
                let json_str = serde_json::to_string(&json_rpc).map_err(|e| {
                    Error::Transport(format!("Failed to serialize JSON-RPC response: {}", e))
                })?;
...
impl Transport for StdioTransport {
    ...
    async fn start(
        ...
        let (stdin_box, stdout_box) =
            if let (Some(stdin_provided), Some(stdout_provided)) = (stdin, stdout) {
                log::debug!(
                    "Using provided stdin/stdout for container {}",
                    container_name
                );
                (stdin_provided, stdout_provided)
            } else {
                log::debug!(
                    "No stdin/stdout provided, attaching to container {}",
                    container_id
                );
                return Err(Error::Transport(
                    "Stdin/stdout must be provided".to_string(),
                ));
            };
...
impl ContainerRuntime for DummyContainerRuntime {
    async fn create_container(
        ...
        Err(Error::Transport(
            "Dummy runtime not implemented".to_string(),
        ))
    }
    ...
    async fn get_container_info(
        &self,
        _container_id: &str,
    ) -> Result<crate::container::ContainerInfo> {
        Err(Error::Transport(
            "Dummy runtime not implemented".to_string(),
        ))
    }
...
mod tests {
    ...
    async fn test_stdio_transport_lifecycle() -> Result<()> {
        ...
        mock_runtime.expect_attach_container().returning(|_| {
        ...
        let stdout = MockAsyncReadWrite::new();
        transport
            .start(Some(Box::new(stdin)), Some(Box::new(stdout)))
            .await?;
    ...
    fn test_json_rpc_message_creation() {
        ...
        assert_eq!(
            error_response.error.as_ref().unwrap().message,
            "Invalid Request"
        );
        assert_eq!(
            error_response
    ...
    fn test_parse_json_rpc_message() {
        ...
        let notification_str =
            r#"{"jsonrpc":"2.0","method":"update","params":{"status":"complete"}}"#;
    ...
    async fn test_process_stdout_chunk() -> Result<()> {
        ...
        // Process the chunk
        StdioTransport::process_stdout_chunk(&transport, data, &mut buffer, &mut line_buffer, &tx)
            .await?;
    ...
    async fn test_send_message_to_container() -> Result<()> {
        // Create a mock stdin
        let mut stdin =
            Box::new(MockAsyncReadWrite::new()) as Box<dyn tokio::io::AsyncWrite + Unpin + Send>;
    ...
    async fn test_process_stdout_chunk_with_partial_lines() -> Result<()> {
        ...
        StdioTransport::process_stdout_chunk(&transport, data2, &mut buffer, &mut line_buffer, &tx)
            .await?;
    ...
    async fn test_send_message_to_container_with_write_error() {
        ...
        let mut stdin =
            Box::new(FailingAsyncWrite) as Box<dyn tokio::io::AsyncWrite + Unpin + Send>;
    ...
    async fn test_send_message_to_container_with_flush_error() {
        // Create a failing stdin
        let mut stdin =
            Box::new(FailingFlushAsyncWrite) as Box<dyn tokio::io::AsyncWrite + Unpin + Send>;
