...
const (
	...
	// AuthServerTLSVolumeName is the volume name for listener TLS credentials.
...
// HasAuthServerListenerTLS reports whether volume mounts enable listener TLS.
func HasAuthServerListenerTLS(volumeMounts []corev1.VolumeMount) bool {
	for _, volumeMount := range volumeMounts {
		if volumeMount.Name == AuthServerTLSVolumeName+"-cert" {
			return true
		}
	}
	return false
...
func buildSPIFFETrustRunConfig(
	...
	for i, domain := range authConfig.SPIFFETrustDomains {
		...
		if domain.BundleSource.File != nil {
			trustDomains[i].BundleSource.File = &authserver.SPIFFEFileBundleSourceRunConfig{
				Path: fmt.Sprintf("%s/%d/%s", AuthServerSPIFFEBundleMountPath, i, AuthServerSPIFFEBundleFileName),
			}
		}
