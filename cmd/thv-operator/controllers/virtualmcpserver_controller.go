...
func (r *VirtualMCPServerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		// Only reconcile on spec changes (metadata.generation change), not status changes.
		// This prevents unnecessary reconciliation when vMCP runtime updates status via StatusReporter.
