...
| `scriptEngine` _[vmcp.config.ScriptEngineConfig](#vmcpconfigscriptengineconfig)_ | ScriptEngine configures the Starlark script execution engine ("code mode").<br />When enabled, clients can call the execute_tool_script virtual tool to<br />run server-side scripts that orchestrate multiple tool calls in a single<br />round-trip, reducing context bloat and inference cycles. |  | Optional: \{\} <br /> |
...
#### vmcp.config.ScriptEngineConfig
