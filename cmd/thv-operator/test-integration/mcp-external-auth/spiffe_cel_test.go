...
var _ = Describe("MCPExternalAuthConfig SPIFFE CEL validation", func() {
	...
	cases := []validationCase{
		...
		{
			name: "empty-string audience is rejected", trustDomains: true, clients: true,
			principal: "spiffe://example.org/ns/default/agent", errMatch: "audiences",
			mutate: func(config *mcpv1beta1.EmbeddedAuthServerConfig) {
				config.InboundGrants.SPIFFEClientAuth[0].Audiences = []string{""}
			},
		},
		{
			name: "empty-string scope is rejected", trustDomains: true, clients: true,
			principal: "spiffe://example.org/ns/default/agent", errMatch: "scopes",
			mutate: func(config *mcpv1beta1.EmbeddedAuthServerConfig) {
				config.InboundGrants.SPIFFEClientAuth[0].Scopes = []string{""}
			},
		},
		{
			name: "malformed resource URI is rejected", trustDomains: true, clients: true,
			principal: "spiffe://example.org/ns/default/agent", errMatch: "resources",
			mutate: func(config *mcpv1beta1.EmbeddedAuthServerConfig) {
				config.InboundGrants.SPIFFEClientAuth[0].Resources = []string{"not-a-url"}
			},
		},
		{
