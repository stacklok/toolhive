...
// Package vmcpcrd holds the CRD-facing mirror of the Virtual MCP Server
// configuration schema.
//
// These types are a deliberate, standalone duplicate of the runtime
// configuration model in pkg/vmcp/config. They exist so that the
// VirtualMCPServer / VirtualMCPCompositeToolDefinition CRD schemas are
// generated exclusively from types owned by the operator API package, and can
// never be perturbed by a change to the internal on-disk/runtime config model.
//
// The decoupling is structural: no field on any type in this package may
// reference a type defined in pkg/vmcp/config. That invariant is what makes the
// no-leak guarantee categorical (controller-gen physically cannot walk into the
// internal package), and it is enforced by a reflection-based test. The
// cmd/thv-operator/pkg/vmcpconfig converter is the single boundary that turns
// these CRD types into the internal config.Config that is marshalled to the
// ConfigMap.
//
// Field-for-field parity with pkg/vmcp/config is enforced by the AssertNoDrift
// tables and a round-trip fuzz test; when one side gains or changes a field,
// those tests fail until the other side and the converter are updated in step.
package vmcpcrd
