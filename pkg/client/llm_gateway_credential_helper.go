...
// would never be found. Single-quoting is a total transform (see
// llmgateway.QuoteForPOSIXShell), so no character in the path needs rejecting
// and no metacharacter validation is required to keep this 0700 script
// injection-free.
//
...
func (cm *ClientManager) writeCredentialHelperShim(tokenHelperPath string) (string, error) {
	...
	quoted := llmgateway.QuoteForPOSIXShell(tokenHelperPath)
	script := "#!/bin/sh\n" +
