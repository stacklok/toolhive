...
//
// The outermost act.sub is always the resolved actor identity: actorID (the
// verified SPIFFE ID for SPIFFE-authenticated clients, or the configured
// OAuth client ID/an actor_token's asserted identity for other clients) —
// every downstream consumer reads that as "who is acting", and it must not
