...
func TestCedarAdmission_InvokesToolAuthorizerCorrectly(t *testing.T) {
	...
	// Trusted backend identity and annotations travel through one metadata channel.
	byName := map[string]mockCall{mock.calls[0].resourceID: mock.calls[0], mock.calls[1].resourceID: mock.calls[1]}
	require.NotNil(t, byName["hinted"].resourceMetadata.Annotations)
	assert.Equal(t, boolPtr(true), byName["hinted"].resourceMetadata.Annotations.ReadOnlyHint)
	assert.Equal(t, "backend-a", byName["hinted"].resourceMetadata.BackendID)
	assert.Nil(t, byName["plain"].resourceMetadata.Annotations)
...
func TestCedarAdmission_BackendScopedPolicy(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, listed[tc.tool.Name], ok,
				"tools/list and tools/call must make the same backend-scoped decision")
		})
	}
}

func TestCedarAdmission_BackendScopedForbidDoesNotApplyToComposite(t *testing.T) {
	t.Parallel()

	adm := cedarAdmissionWith(t,
		`permit(principal, action == Action::"call_tool", resource);`,
		`forbid(principal, action == Action::"call_tool", resource in Backend::"backend-a");`,
	)
	id := cedarIdentity()
	ctx := context.Background()

	forbiddenBackend := vmcp.Tool{Name: "charge", BackendID: "backend-a"}
	otherBackend := vmcp.Tool{Name: "search", BackendID: "backend-b"}
	composite := vmcp.Tool{Name: "deploy-and-charge"}
	tools := []vmcp.Tool{forbiddenBackend, otherBackend, composite}

	got, err := adm.FilterTools(ctx, id, tools)
	require.NoError(t, err)
	assert.Equal(t, []string{"search", "deploy-and-charge"}, toolNames(got),
		"a composite has no Backend parent, so only the broader permit applies")

	want := map[string]bool{
		"charge":            false,
		"search":            true,
		"deploy-and-charge": true,
	}
	for i := range tools {
		tool := &tools[i]
		t.Run(tool.Name, func(t *testing.T) {
			...
			ok, err := adm.AllowToolCall(ctx, id, tool, nil)
			require.NoError(t, err)
			assert.Equal(t, want[tool.Name], ok)
		})
	}
}
