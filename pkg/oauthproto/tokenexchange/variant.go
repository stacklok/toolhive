...
type VariantHandler interface {
	...
	// HTTP response has been parsed into a TokenResponse struct. Implementations
	// should return an error if required fields are missing or invalid.
	ValidateResponse(resp *oauthproto.TokenResponse) error
...
// "actor_token"/"actor_token_type" (Section 2.1) parameters. ExchangeConfig
// has a Resource field — emit it when set. ActingParty is not yet exposed
// at the ExchangeConfig layer; when added, this method should be updated.
func (*rfc8693Handler) BuildFormData(config *ExchangeConfig, subjectToken string) (url.Values, error) {
	...
	data.Set("grant_type", oauthproto.GrantTypeTokenExchange)
	...
	// Requested token type defaults to access_token per RFC 8693.
	...
	if config.Resource != "" {
		data.Set("resource", config.Resource)
	}
...
func (*rfc8693Handler) ValidateResponse(resp *oauthproto.TokenResponse) error {
	if resp == nil {
