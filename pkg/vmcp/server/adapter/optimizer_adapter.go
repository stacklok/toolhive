...
// allowing it to work with any optimizer implementation without direct dependency.
//
// Deprecated: Use CreateOptimizerToolsFromProvider for new code.
// This signature is kept for backward compatibility with existing server code.
func CreateOptimizerTools(provider OptimizerHandlerProvider) []server.ServerTool {
	if provider == nil {
		// Return empty slice for nil provider (backward compat with old behavior)
		return nil
	}
...
// CreateOptimizerToolsFromProvider creates optimizer tools with error handling.
// This is the preferred function for new code.
func CreateOptimizerToolsFromProvider(provider OptimizerHandlerProvider) ([]server.ServerTool, error) {
	if provider == nil {
		return nil, fmt.Errorf("optimizer handler provider cannot be nil")
	}

	return CreateOptimizerTools(provider), nil
}
