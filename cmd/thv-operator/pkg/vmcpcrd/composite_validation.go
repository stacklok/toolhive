...
// This file is a deliberate duplicate of the composite-tool validation in
// pkg/vmcp/config/composite_validation.go, operating on the vmcpcrd mirror
// types. It exists so that the VirtualMCPServer / VirtualMCPCompositeToolDefinition
// admission webhooks can validate composite tools without api/v1beta1 importing
// pkg/vmcp/config (which would re-couple the CRD schema to the internal model).
// Keep it behaviourally in lock-step with the runtime copy.
