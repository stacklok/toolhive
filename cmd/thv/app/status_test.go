	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			output := captureStdout(t, func() {
				printStatusTextOutput(tt.workload, rt.WorkloadStats{})
			})
...
func TestPrintStatusJSONOutput(t *testing.T) {
	...
	var jsonErr error
	output := captureStdout(t, func() {
		jsonErr = printStatusJSONOutput(workload, rt.WorkloadStats{
	...
	if parsed.CPUUsage != "5.00%" {
		t.Errorf("CPUUsage mismatch: got %q, want %q", parsed.CPUUsage, "5.00%")
	}
	if parsed.MemoryUsage != "50.0 MB" {
		t.Errorf("MemoryUsage mismatch: got %q, want %q", parsed.MemoryUsage, "50.0 MB")
	}
}
