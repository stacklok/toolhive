...
condition reports `Reason: UnsupportedAuthType`. vMCP also reports the affected
backend condition and excludes that backend from the usable set in both static
and dynamic discovery modes. If an explicitly configured default strategy is
invalid, only backends with a successfully resolved backend-specific strategy
remain routable; vMCP does not fall back to unauthenticated access.
