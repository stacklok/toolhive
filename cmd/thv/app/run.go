...
func init() {
	runCmd.Flags().StringVar(&runTransport, "transport", "stdio", "Transport mode (sse, http-stream or stdio)")
...
func applyRegistrySettings(
	...
	if !cmd.Flags().Changed("target-port") && (server.Transport == "sse" || server.Transport == "http-stream") &&
		server.TargetPort > 0 {
