...
impl SseTransport {
    ...
    async fn handle_request(
        ...
        headers.insert(
            "X-Forwarded-Host",
            req.uri().host().unwrap_or("localhost").parse().unwrap(),
        );
        ...
        // Log the response
        println!(
            "Received response from container {}: status {}",
            container_id,
            res.status()
        );
...
impl Transport for SseTransport {
    ...
    async fn setup(
        ...
        *self.container_name.lock().unwrap() = Some(container_name.to_string());
        ...
        println!(
            "SSE transport setup for container {} on port {}",
    ...
    async fn start(
        ...
        let make_svc = make_service_fn(move |_| {
            ...
            async move {
                Ok::<_, hyper::Error>(service_fn(move |req: Request<Body>| {
                    ...
                    async move {
                        match Self::handle_request(req, container_port, container_id, container_ip)
                            .await
                        {
        ...
        println!(
            "Reverse proxy started for container {} on port {}",
            container_name, self.port
        );
...
mod tests {
    ...
    async fn test_sse_transport_setup() {
        ...
        assert_eq!(
            transport.container_ip.lock().unwrap().as_ref().unwrap(),
            "172.17.0.2"
        );

        // Check that container ID and name were set correctly
        assert_eq!(
            transport.container_id.lock().unwrap().as_ref().unwrap(),
            "test-id"
        );
        assert_eq!(
            transport.container_name.lock().unwrap().as_ref().unwrap(),
            "test-container"
        );
    }
