...
func (f *defaultMultiSessionFactory) makeSession(
	...
	baseSession := f.makeBaseSession(ctx, sessID, identity, iss, sub, backends, nil, sink)
...
func (f *defaultMultiSessionFactory) RestoreSession(
	...
	// path only). The restored (iss, sub) from the parsed binding feeds the
	// untrusted resolver, which finds the session's existing pods by
	// deterministic name.
	baseSession := f.makeBaseSession(ctx, id, nil, restoredIss, restoredSub, filteredBackends, sessionHints, nil)
