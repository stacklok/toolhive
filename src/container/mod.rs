...
impl ContainerMonitor {
    ...
    pub fn new(runtime: Box<dyn ContainerRuntime>, container_id: &str, container_name: &str) -> Self {
        Self {
    ...
    pub async fn start_monitoring(&mut self) -> Result<mpsc::Receiver<Error>> {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let is_running = match runtime.lock().await.is_container_running(&container_id).await {
                    Ok(running) => running,
                    Err(e) => {
                ...
                if !is_running {
                    ...
                    let exit_info = match runtime.lock().await.get_container_info(&container_id).await {
...
mod tests {
    ...
    async fn test_container_monitor_detects_exit() -> Result<()> {
        ...
        assert!(error.is_ok(), "Timeout waiting for container exit notification");
        ...
        if let Some(Error::ContainerExited(msg)) = error {
            assert!(msg.contains("exited unexpectedly"), "Error message should indicate unexpected exit");
            assert!(msg.contains("container1"), "Error message should contain container ID");
            assert!(msg.contains("test-container"), "Error message should contain container name");
        } else {
