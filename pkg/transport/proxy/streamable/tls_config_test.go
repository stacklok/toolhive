...
func TestWithTLSConfigClonesConfiguration(t *testing.T) {
	...
	proxy := NewHTTPProxy("127.0.0.1", 0, nil, nil, WithTLSConfig(original))
...
func TestHTTPProxyTLSListener(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			proxy := NewHTTPProxy("127.0.0.1", port, nil, nil, options...)
			...
			url := fmt.Sprintf("%s://127.0.0.1:%d/ready", tt.scheme, port)
