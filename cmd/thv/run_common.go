...
func detachProcess(cmd *cobra.Command, options *runner.RunConfig) error {
	...
	logFilePath, err := xdg.DataFile(fmt.Sprintf("toolhive/logs/%s.log", options.BaseName))
	if err != nil {
	...
	if err != nil {
		logger.Log.Warn(fmt.Sprintf("Warning: Failed to create log file: %v", err))
	} else {
		defer logFile.Close()
		logger.Log.Info(fmt.Sprintf("Logging to: %s", logFilePath))
	}
