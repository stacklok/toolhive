...
func staticClient(t *testing.T, grants []string) *registration.SPIFFEClient {
	...
	client, err := registration.NewSPIFFEClient(
		"client", grants, []string{"openid"}, []string{"https://audience.example.com"}, []string{"https://resource.example.com"},
	)
	require.NoError(t, err)
	return client
}
...
func authenticatedClient(t *testing.T, grants []string) *registration.AuthenticatedSPIFFEClient {
	...
	principal := spiffeauth.NewNormalizedSPIFFEPrincipal(
		...
		spiffeauth.NewSPIFFEAuthorizationPolicy(
			grants, []string{"openid"}, []string{"https://resource.example.com"}, []string{"https://audience.example.com"}, false,
		),
	)
...
func TestHandlerProviderIssuesEquivalentSPIFFEClientCredentialsTokens(t *testing.T) {
	...
	for _, method := range []spiffeauth.SPIFFEAuthenticationMethod{
		...
		t.Run(string(method), func(t *testing.T) {
			...
			config, key := newSPIFFEClientCredentialsConfig(t, issuer)
			...
			provider := newSPIFFEClientCredentialsProvider(t, config, key)
...
func newSPIFFEClientCredentialsConfig(t *testing.T, issuer string) (*server.AuthorizationServerConfig, *rsa.PrivateKey) {
	t.Helper()
...
func newSPIFFEClientCredentialsProvider(
	t *testing.T, config *server.AuthorizationServerConfig, key *rsa.PrivateKey,
) fosite.OAuth2Provider {
	t.Helper()

	store := storage.NewMemoryStorage()
	t.Cleanup(func() { _ = store.Close() })
	jwk := &josev3.JSONWebKey{Key: key, KeyID: "test-key", Algorithm: "RS256", Use: "sig"}
	strategy := &compose.CommonStrategy{CoreStrategy: compose.NewOAuth2JWTStrategy(
		func(context.Context) (interface{}, error) { return jwk, nil },
...
func newAuthenticatedCarrier(
	...
	client, err := registration.NewSPIFFEClient(
		clientID, []string{oauthproto.GrantTypeClientCredentials}, scopes, []string{"https://audience.example.com"}, []string{resource},
	)
	require.NoError(t, err)
	principal := spiffeauth.NewNormalizedSPIFFEPrincipal(
		clientID, spiffeID, "example.org", method,
		spiffeauth.NewSPIFFEAuthorizationPolicy(
			[]string{oauthproto.GrantTypeClientCredentials}, scopes, []string{resource}, []string{"https://audience.example.com"}, false,
		),
	)
