...
func TestGenerateAuthServerEnvVars(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantEnvNames: []string{UpstreamClientSecretEnvVar + "_0"},
		},
		{
		...
		},
		{
			...
			wantEnvNames: []string{
				UpstreamClientSecretEnvVar + "_0",
				UpstreamClientSecretEnvVar + "_1",
			},
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			checkFunc: func(t *testing.T, config *authserver.RunConfig) {
				...
				assert.Equal(t, UpstreamClientSecretEnvVar+"_0", okta.OIDCConfig.ClientSecretEnvVar)
				...
				assert.Equal(t, UpstreamClientSecretEnvVar+"_1", github.OAuth2Config.ClientSecretEnvVar)
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantUpstreamCS {
				_, ok := envMap[UpstreamClientSecretEnvVar+"_0"]
