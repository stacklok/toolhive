...
func TestUpgrade_ProvenanceFieldDivergenceBlocked(t *testing.T) {
	tests := []struct {
		...
		lockedRunner   string
		candidateRef   string
		candidateRunne string
		description    string
	}{
		{
			name:      "attacker branch",
			lockedRef: "refs/heads/main", candidateRef: "refs/heads/attacker",
			lockedRunner: testRunnerEnvironment, candidateRunne: testRunnerEnvironment,
		...
		},
		{
			...
			lockedRunner: testRunnerEnvironment, candidateRunne: "self-hosted",
			...
			mv.EXPECT().VerifyGit(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
				...
				DoAndReturn(func(_ any, _, _ []byte, expected *lockfile.Provenance) (*verifier.Result, error) {
					...
					if expected == nil {
						...
						return provenanceSignedResult(tc.candidateRef, tc.candidateRunne), nil
					}
...
func TestUpgrade_UnaffectedEntrySkipsSignerProbe(t *testing.T) {
	...
	mv.EXPECT().VerifyGit(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
		AnyTimes().
		DoAndReturn(func(_ any, _, _ []byte, _ *lockfile.Provenance) (*verifier.Result, error) {
			calls++
			return nil, verifier.ErrUnsigned
		})
	mv.EXPECT().VerifyBundleOffline(gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return(nil)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			mv.EXPECT().VerifyOCI(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
				DoAndReturn(func(_ any, _, _ string, expected *lockfile.Provenance) (*verifier.Result, error) {
