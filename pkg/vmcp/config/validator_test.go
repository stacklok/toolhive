...
func TestValidator_ValidateOptimizer(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name    string
		opt     *OptimizerConfig
		...
		{
			name: "boundary: hybridSearchSemanticRatio = 1.0",
			opt: &OptimizerConfig{
				EmbeddingService:          "http://example.com:8080",
		...
		{
			name: "boundary: semanticDistanceThreshold = 2.0",
			opt: &OptimizerConfig{
				EmbeddingService:          "http://example.com:8080",
		...
		{
			name: "maxToolsToReturn too high",
			opt: &OptimizerConfig{
				EmbeddingService: "http://example.com:8080",
				MaxToolsToReturn: 51,
			},
			wantErr: true,
			errMsg:  "optimizer.maxToolsToReturn must be between 1 and 50",
		},
		{
			name: "maxToolsToReturn negative",
			opt: &OptimizerConfig{
				EmbeddingService: "http://example.com:8080",
				MaxToolsToReturn: -1,
			},
		...
		{
			name: "hybridSearchSemanticRatio too high",
			opt: &OptimizerConfig{
				EmbeddingService:          "http://example.com:8080",
				HybridSearchSemanticRatio: "1.1",
			},
			wantErr: true,
			errMsg:  "optimizer.hybridSearchSemanticRatio must be between 0.0 and 1.0",
		},
		{
			name: "hybridSearchSemanticRatio negative",
			opt: &OptimizerConfig{
				EmbeddingService:          "http://example.com:8080",
		...
		{
			name: "semanticDistanceThreshold too high",
			opt: &OptimizerConfig{
				EmbeddingService:          "http://example.com:8080",
				SemanticDistanceThreshold: "2.1",
			},
			wantErr: true,
			errMsg:  "optimizer.semanticDistanceThreshold must be between 0.0 and 2.0",
		},
		{
			name: "semanticDistanceThreshold negative",
			opt: &OptimizerConfig{
				EmbeddingService:          "http://example.com:8080",
				SemanticDistanceThreshold: "-0.1",
			},
			wantErr: true,
			errMsg:  "optimizer.semanticDistanceThreshold must be between 0.0 and 2.0",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			v := NewValidator()
			err := v.validateOptimizer(tt.opt)

			if (err != nil) != tt.wantErr {
				t.Errorf("validateOptimizer() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if tt.wantErr && err != nil && tt.errMsg != "" {
				if !strings.Contains(err.Error(), tt.errMsg) {
					t.Errorf("validateOptimizer() error message = %v, want to contain %v", err.Error(), tt.errMsg)
				}
			}
		})
	}
