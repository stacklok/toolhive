...
func newReadinessHTTPClient(tlsConfigs ...*TLSConfig) (*http.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("load readiness TLS certificate and key: %w", err)
	}
	if len(certificate.Certificate) == 0 {
		return nil, errors.New("parse readiness TLS certificate: certificate chain is empty")
	}
	configuredLeaf, err := x509.ParseCertificate(certificate.Certificate[0])
	if err != nil {
		return nil, fmt.Errorf("parse readiness TLS serving certificate: %w", err)
	}

	roots := x509.NewCertPool()
	roots.AddCert(configuredLeaf)
	httpClient.Transport = &http.Transport{
		TLSClientConfig: &tls.Config{
			MinVersion:         tls.VersionTLS12,
			InsecureSkipVerify: true, //nolint:gosec // Hostname verification is replaced by exact leaf pinning and x509 verification below.
			VerifyConnection: func(state tls.ConnectionState) error {
				if len(state.PeerCertificates) == 0 {
					return errors.New("verify readiness TLS connection: peer did not provide a certificate")
				}
				peerLeaf := state.PeerCertificates[0]
				if !peerLeaf.Equal(configuredLeaf) {
					return errors.New("verify readiness TLS connection: peer certificate does not match configured serving certificate")
				}
				if _, err := peerLeaf.Verify(x509.VerifyOptions{
					Roots:     roots,
					KeyUsages: []x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth},
				}); err != nil {
					return fmt.Errorf("verify readiness TLS certificate validity and server authentication usage: %w", err)
				}
				return nil
			},
		},
