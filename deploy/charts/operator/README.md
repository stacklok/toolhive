...
| operator | object | `{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":100,"minReplicas":1,"targetCPUUtilizationPercentage":80},"containerSecurityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"readOnlyRootFilesystem":true,"runAsNonRoot":true,"runAsUser":1000,"seccompProfile":{"type":"RuntimeDefault"}},"env":{},"features":{"experimental":false,"registry":true,"server":true,"virtualMCP":true},"gc":{"gogc":75,"gomeglimit":"150MiB"},"image":"ghcr.io/stacklok/toolhive/operator:v0.9.3","imagePullPolicy":"IfNotPresent","imagePullSecrets":[],"leaderElectionRole":{"binding":{"name":"toolhive-operator-leader-election-rolebinding"},"name":"toolhive-operator-leader-election-role","rules":[{"apiGroups":[""],"resources":["configmaps"],"verbs":["get","list","watch","create","update","patch","delete"]},{"apiGroups":["coordination.k8s.io"],"resources":["leases"],"verbs":["get","list","watch","create","update","patch","delete"]},{"apiGroups":[""],"resources":["events"],"verbs":["create","patch"]}]},"livenessProbe":{"httpGet":{"path":"/healthz","port":"health"},"initialDelaySeconds":15,"periodSeconds":20},"nodeSelector":{},"podAnnotations":{},"podLabels":{},"podSecurityContext":{"runAsNonRoot":true},"ports":[{"containerPort":8080,"name":"metrics","protocol":"TCP"},{"containerPort":8081,"name":"health","protocol":"TCP"}],"proxyHost":"0.0.0.0","rbac":{"allowedNamespaces":[],"scope":"cluster"},"readinessProbe":{"httpGet":{"path":"/readyz","port":"health"},"initialDelaySeconds":5,"periodSeconds":10},"replicaCount":1,"resources":{"limits":{"cpu":"500m","memory":"128Mi"},"requests":{"cpu":"10m","memory":"64Mi"}},"serviceAccount":{"annotations":{},"automountServiceAccountToken":true,"create":true,"labels":{},"name":"toolhive-operator"},"tolerations":[],"toolhiveRunnerImage":"ghcr.io/stacklok/toolhive/proxyrunner:v0.9.3","vmcpImage":"ghcr.io/stacklok/toolhive/vmcp:v0.9.3","volumeMounts":[],"volumes":[],"webhook":{"enabled":false,"failurePolicy":"Fail"}}` | All values for the operator deployment and associated resources |
...
| operator.image | string | `"ghcr.io/stacklok/toolhive/operator:v0.9.3"` | Container image for the operator |
...
| operator.toolhiveRunnerImage | string | `"ghcr.io/stacklok/toolhive/proxyrunner:v0.9.3"` | Image to use for Toolhive runners |
| operator.vmcpImage | string | `"ghcr.io/stacklok/toolhive/vmcp:v0.9.3"` | Image to use for Virtual MCP Server (vMCP) deployments |
...
| operator.webhook | object | `{"enabled":false,"failurePolicy":"Fail"}` | Webhook configuration for admission validation |
| operator.webhook.enabled | bool | `false` | Enable webhook server and admission validation. When enabled, the operator will generate self-signed certificates and create ValidatingWebhookConfiguration for resources with validation webhooks. Currently validates: VirtualMCPServer, VirtualMCPCompositeToolDefinition, and MCPExternalAuthConfig. Webhook handlers are only registered for resources whose feature controllers are enabled. NOTE: Disabled by default. Enable explicitly when webhook support is needed. |
| operator.webhook.failurePolicy | string | `"Fail"` | Failure policy for webhooks. Can be "Fail" (block invalid resources) or "Ignore" (allow invalid resources). |
