...
type BackendHealthStatusResponse struct {
	...
	// Backends is a map of backend ID to health state
	Backends map[string]struct {
		Status              string    `json:"status"`
		ConsecutiveFailures int       `json:"consecutiveFailures"`
		LastCheckTime       time.Time `json:"lastCheckTime"`
		LastTransitionTime  time.Time `json:"lastTransitionTime"`
		// Note: LastError is omitted because Go error types don't unmarshal cleanly from JSON
		// We only need Status and LastCheckTime for backend health tracking
...
func (r *VirtualMCPServerReconciler) queryVMCPHealthStatus(
	...
	for backendID, backend := range healthResp.Backends {
		healthStatus[backendID] = &BackendHealthInfo{
