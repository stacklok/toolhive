...
func newAuthenticatedSPIFFEClientWithAudiences(t *testing.T, resources, audiences, scopes, grantTypes []string) *registration.AuthenticatedSPIFFEClient {
	...
	// registration.NewSPIFFEClient requires non-empty scopes and audiences
	// (an association's Scopes/Audiences are required config fields; only
	// Resources is optional). Tests that only care about resource-based
	// enforcement pass nil for either and get an unused placeholder here,
	// matching real-world associations rather than the impossible
	// "no audience at all" case.
	if len(scopes) == 0 {
		scopes = []string{"openid"}
	}
	if len(audiences) == 0 {
		audiences = []string{"https://unused-audience.example.com"}
	}

	static, err := registration.NewSPIFFEClient(testAgentClientID, grantTypes, scopes, audiences, resources)
	require.NoError(t, err)
