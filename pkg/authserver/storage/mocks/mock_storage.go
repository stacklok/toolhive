...
//
//	mockgen -destination=mocks/mock_storage.go -package=mocks -source=types.go Storage,PendingAuthorizationStorage,AssertionJWTConsumer,ClientRegistry,ConfiguredClientReconciler,UpstreamTokenStorage,UpstreamTokenRefresher,UserStorage,DCRCredentialStore
//
...
// MockConfiguredClientReconciler is a mock of ConfiguredClientReconciler interface.
type MockConfiguredClientReconciler struct {
	ctrl     *gomock.Controller
	recorder *MockConfiguredClientReconcilerMockRecorder
	isgomock struct{}
}

// MockConfiguredClientReconcilerMockRecorder is the mock recorder for MockConfiguredClientReconciler.
type MockConfiguredClientReconcilerMockRecorder struct {
	mock *MockConfiguredClientReconciler
}

// NewMockConfiguredClientReconciler creates a new mock instance.
func NewMockConfiguredClientReconciler(ctrl *gomock.Controller) *MockConfiguredClientReconciler {
	mock := &MockConfiguredClientReconciler{ctrl: ctrl}
	mock.recorder = &MockConfiguredClientReconcilerMockRecorder{mock}
	return mock
}

// EXPECT returns an object that allows the caller to indicate expected use.
func (m *MockConfiguredClientReconciler) EXPECT() *MockConfiguredClientReconcilerMockRecorder {
	return m.recorder
}

// ReconcileConfiguredClients mocks base method.
func (m *MockConfiguredClientReconciler) ReconcileConfiguredClients(ctx context.Context, clients []fosite.Client) error {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "ReconcileConfiguredClients", ctx, clients)
	ret0, _ := ret[0].(error)
	return ret0
}

// ReconcileConfiguredClients indicates an expected call of ReconcileConfiguredClients.
func (mr *MockConfiguredClientReconcilerMockRecorder) ReconcileConfiguredClients(ctx, clients any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "ReconcileConfiguredClients", reflect.TypeOf((*MockConfiguredClientReconciler)(nil).ReconcileConfiguredClients), ctx, clients)
}
