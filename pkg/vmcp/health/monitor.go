...
func (m *Monitor) convertToDiscoveredBackends(allStates map[string]*State) []vmcp.DiscoveredBackend {
	...
	for backendID, state := range allStates {
		...
		if !exists {
			// Backend not in current list - this can happen during a race condition
			// where a health check completes just after RemoveBackend() is called.
			// Use the backend ID as the name and empty URL as fallback.
			discoveredBackends = append(discoveredBackends, vmcp.DiscoveredBackend{
				Name:            backendID, // Use ID as name for removed backends
				URL:             "",        // No URL for removed backends
