...
```
  ...
  -l, --label stringArray                          Set labels on the container (format: key=value)
      ...
      --remote-auth-bearer-token string            Bearer token for remote server authentication
      --remote-auth-bearer-token-env-var string    Environment variable name containing bearer token (alternative to --remote-auth-bearer-token)
