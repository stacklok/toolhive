...
import (
	...
	"github.com/stacklok/toolhive/pkg/registry/servers"
...
const verifyImageDefault = servers.VerifyImageWarn
...
func init() {
	...
	runCmd.Flags().StringVar(
		...
		"image-verification",
		verifyImageDefault,
		fmt.Sprintf("Set image verification mode (%s, %s, %s)", servers.VerifyImageWarn, servers.VerifyImageEnabled, servers.VerifyImageDisabled),
	)
...
func runCmdFunc(cmd *cobra.Command, args []string) error {
	...
	// Get the imageMetadata name or image
	...
	imageURL, imageMetadata, err := servers.GetMCPServer(ctx, serverOrImage, runCACertPath, runVerifyImage)
	if err != nil {
	...
	if runForeground {
		return manager.RunWorkload(ctx, runConfig)
	}
	return manager.RunWorkloadDetached(runConfig)
}
