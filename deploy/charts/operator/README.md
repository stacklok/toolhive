...
| operator.rbac | object | `{"allowedNamespaces":[],"disableWorkloadRBAC":false,"scope":"cluster"}` | RBAC configuration for the operator |
...
| operator.rbac.disableWorkloadRBAC | bool | `false` | Disable per-workload RBAC management (ServiceAccount, Role, RoleBinding). When true, the operator will not create RBAC resources for workloads. Use this when RBAC is managed externally (e.g., via per-workload Helm charts). |
