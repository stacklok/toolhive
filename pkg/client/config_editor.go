...
func (jcu *JSONConfigUpdater) Upsert(serverName string, data MCPServer) error {
	...
	if err != nil {
		logger.Log.Error("Failed to read file: %v", err)
	}
	...
	if err != nil {
		logger.Log.Error("Unable to marshal the MCPServer into JSON: %v", err)
	}

	patch := fmt.Sprintf(`[{ "op": "add", "path": "%s/%s", "value": %s } ]`, jcu.MCPServersPathPrefix, serverName, dataJSON)
	err = v.Patch([]byte(patch))
	if err != nil {
		logger.Log.Error("Failed to patch file: %v", err)
	}

	formatted, _ := hujson.Format(v.Pack())
	if err != nil {
		logger.Log.Error("Failed to format the patched file: %v", err)
	}
...
func (jcu *JSONConfigUpdater) Remove(serverName string) error {
	...
	logger.Log.Info("Successfully removed the MCPServer %s from the client config file", serverName)
