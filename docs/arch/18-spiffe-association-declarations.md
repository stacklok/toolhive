...
**Current status: rejected at startup, not deployable yet.** `RunConfig.Validate()` hard-fails on any non-empty `spiffeTrustDomains`/`inboundGrants.spiffeClientAuth` before the runner is created (`validateSPIFFENotYetEnforced` in `pkg/authserver/config.go`). X.509-SVID and JWT-SVID verification consumers now exist, but `newServer` does not construct their trust-bundle sources. Issue [#6201](https://github.com/stacklok/toolhive/issues/6201) must wire and rotate those sources and remove the startup guard before SPIFFE client authentication can run in a deployment.

The embedded authorization server carries SPIFFE trust declarations and client associations across the `RunConfig` boundary. It also implements credential verification and association resolution for X.509-SVIDs and JWT-SVIDs. These paths fail closed without a configured bundle source; declaring SPIFFE configuration does not fetch or load trust material.

## Model and boundaries

Configuration separates top-level trust declarations from canonical client associations:

- `spiffe_trust_domains` in `RunConfig` (`spiffeTrustDomains` in the CRD) names a canonical SPIFFE trust domain, explicitly lists permitted methods (`spiffe_x509` and/or `spiffe_jwt`), and declares the source that #6201 will use to load its trust bundle.
...
### Trust-bundle source (declared, not yet wired)

Every trust domain must declare exactly one `bundle_source`, a discriminated union naming where the bundle loader in [#6201](https://github.com/stacklok/toolhive/issues/6201) will get trust material. The declaration is validated for shape, but startup does not yet construct the `x509bundle.Source` or `jwtbundle.Source` consumed by credential verification:
...
The X.509 consumer is implemented for the token endpoint. When an association enables `spiffe_x509`, the proxy listener requests a client certificate and middleware extracts its claimed SPIFFE ID only for `/oauth/token`. The authentication strategy then independently re-verifies the presented chain against the current `x509bundle.Source` and resolves the verified identity through the same association registry used by JWT-SVID authentication. This second verification keeps trust decisions tied to the current bundle rather than TLS state established before a bundle rotation.

Validation is deliberately stricter than ordinary mTLS. The leaf must contain exactly one URI SAN, that SAN must be a SPIFFE ID with a non-root canonical path, and mixed SPIFFE/non-SPIFFE URI SANs are rejected. The leaf must not be a CA, must allow digital signatures, and must not allow certificate or CRL signing. If an extended-key-usage extension is present, it must include both server and client authentication. go-spiffe verifies the complete presented chain against the bundle selected by the leaf ID's trust domain. Missing bundle sources, certificates, associations, or method authorization all fail closed.

X.509 authentication requires one exact, non-empty `client_id`; it must be owned by the verified association. HTTP Basic credentials, `client_secret`, and client-assertion fields cannot be mixed with the ambient mTLS identity. Failures return generic OAuth errors and do not log certificate material.

The serving TLS configuration applies to the **entire client-facing proxy listener**, not only auth-server routes. Native SSE, native Streamable HTTP, and both stdio HTTP proxy modes therefore serve HTTPS when listener TLS is configured. `RunConfig.tls_config` names the serving `cert_file` and `key_file`; the runner validates the initial pair, requires TLS when an X.509 association is present, and reloads the pair during every TLS handshake. In Kubernetes, `EmbeddedAuthServerConfig.listenerTLS` projects the referenced Secret keys into a directory mount so kubelet updates remain visible without a pod restart. MCPServer and MCPRemoteProxy publish HTTPS status URLs and use HTTPS startup, readiness, and liveness probes in this mode.

`VirtualMCPServer` does not implement this listener path. Its API validation and controller explicitly reject `authServerConfig.listenerTLS`; consequently SPIFFE X.509 client authentication is also unsupported there.

## JWT-SVID client authentication

The dispatch and validation logic for JWT-SVID client authentication is implemented for configured associations, though it is not yet reachable end to end (see "Security and delivery scope" below). The immutable dispatcher selects the SPIFFE JWT arm whenever any `client_assertion_type` form value is the SPIFFE JWT type — the shared dispatcher itself is otherwise untouched, so an entirely non-SPIFFE request (including RFC 7523 private-key JWT) still reaches Fosite's default strategy unchanged. Within the SPIFFE JWT arm, a duplicate or otherwise ambiguous `client_assertion_type` is rejected. Absent any SPIFFE-selecting value, the request falls through to Fosite's default strategy unless an ambient SPIFFE X.509 identity is present; that identity selects the fail-closed X.509 verification arm instead.
...
Configuration and loaded bundles are not authentication by themselves. A client ID, declared association, request header, unverified SPIFFE-looking URI, client-supplied trust domain, or loaded bundle is never workload identity. The X.509 path establishes identity only after strict certificate-profile and chain verification against the configured trust material; the JWT path does so only after assertion verification. Both then require the association registry to authorize the verified SPIFFE ID, authentication method, and configured client ID.

The X.509-SVID and JWT-SVID consumers are implemented, but `newServer` (`pkg/authserver/server_impl.go`) does not yet construct either bundle source. Issue [#6201](https://github.com/stacklok/toolhive/issues/6201) will load and rotate trust bundles, supply both sources, and remove the `RunConfig.Validate()` startup guard. Until then, every non-empty SPIFFE configuration is rejected before listener or authorization-server startup, so the feature is not deployable. The following remain separate and pending:
