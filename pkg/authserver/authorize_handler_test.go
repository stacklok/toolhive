...
// mockUpstreamProvider implements UpstreamProvider for testing.
...
func (m *mockUpstreamProvider) Name() string {
	return m.name
}

func (m *mockUpstreamProvider) AuthorizationURL(state, _ string, scopes []string) (string, error) {
...
func (m *mockUpstreamProvider) ExchangeCode(_ context.Context, code, _ string) (*IDPTokens, error) {
	m.capturedCode = code
...
func (m *mockUpstreamProvider) RefreshTokens(_ context.Context, _ string) (*IDPTokens, error) {
	if m.refreshErr != nil {
...
func (m *mockUpstreamProvider) UserInfo(_ context.Context, _ string) (*UserInfo, error) {
...
func authorizeTestSetup(t *testing.T) (*Router, *MemoryStorage, *mockUpstreamProvider) {
	t.Helper()
	...
	mockUpstream := &mockUpstreamProvider{
		name:             "test-idp",
	...
	router := NewRouter(logger, provider, oauth2Config, storage, WithUpstreamProvider(mockUpstream))
...
func TestAuthorizeHandler_NoUpstreamProvider(t *testing.T) {
	t.Parallel()
	router, _, _ := authorizeTestSetup(t)
...
func TestCallbackHandler_NoUpstreamProvider(t *testing.T) {
	t.Parallel()
	router, storage, _ := authorizeTestSetup(t)
	// Remove upstream provider
	router.upstream = nil
