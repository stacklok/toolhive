...
func (te *testEngine) expectToolCall(toolName string, args, output map[string]any) {
	...
	te.Backend.EXPECT().CallTool(gomock.Any(), gomock.Any(), target, toolName, args, gomock.Any()).Return(result, nil)
}
...
func (te *testEngine) expectToolCallWithError(toolName string, args map[string]any, err error) {
	...
	te.Router.EXPECT().RouteTool(gomock.Any(), toolName).Return(target, nil)
	te.Backend.EXPECT().CallTool(gomock.Any(), gomock.Any(), target, toolName, args, gomock.Any()).Return(nil, err)
}
...
func (te *testEngine) expectToolCallWithAnyArgsAndError(toolName string, err error) {
	...
	te.Router.EXPECT().RouteTool(gomock.Any(), toolName).Return(target, nil)
	te.Backend.EXPECT().CallTool(gomock.Any(), gomock.Any(), target, toolName, gomock.Any(), gomock.Any()).Return(nil, err)
}
...
func (te *testEngine) expectToolCallWithAnyArgs(toolName string, output map[string]any) {
	...
	te.Backend.EXPECT().CallTool(gomock.Any(), gomock.Any(), target, toolName, gomock.Any(), gomock.Any()).Return(result, nil)
}
