...
func (s *WorkloadRoutes) updateWorkload(w http.ResponseWriter, r *http.Request) {
	...
	currentWorkload, err := s.workloadManager.GetWorkload(ctx, name)
	if err != nil {
		logger.Errorf("Failed to get workload: %v", err)
		http.Error(w, "Workload not found", http.StatusNotFound)
		return
	...
	runConfig, err := s.workloadService.UpdateWorkloadFromRequest(ctx, name, &createReq, currentWorkload.Port)
	if err != nil {
