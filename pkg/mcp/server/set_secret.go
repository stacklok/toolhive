...
func (h *Handler) SetSecret(ctx context.Context, request *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Validate arguments
	if args.Name == "" {
		return NewToolResultError("Secret name cannot be empty"), nil
	}
	if args.FilePath == "" {
		return NewToolResultError("File path cannot be empty"), nil
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return NewToolResultError(fmt.Sprintf("File does not exist: %s", cleanPath)), nil
		}
		return NewToolResultError(fmt.Sprintf("Cannot access file: %v", err)), nil
	}

	// Check if it's a regular file (not a directory)
	if !fileInfo.Mode().IsRegular() {
		return NewToolResultError(fmt.Sprintf("Path is not a regular file: %s", cleanPath)), nil
	}

	// Check file size (limit to 1MB for safety)
	const maxFileSize = 1024 * 1024 // 1MB
	if fileInfo.Size() > maxFileSize {
		return NewToolResultError(fmt.Sprintf("File too large (max %d bytes): %d bytes", maxFileSize, fileInfo.Size())), nil
	}
	...
	if secretValue == "" {
		return NewToolResultError("File content is empty or contains only whitespace"), nil
	}
	...
	if err := secretsProvider.SetSecret(ctx, args.Name, secretValue); err != nil {
		return NewToolResultError(fmt.Sprintf("Failed to set secret: %v", err)), nil
	}
