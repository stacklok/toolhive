...
func (d *DetachedEnvVarValidator) Validate(
	_ context.Context,
	metadata *registry.ImageMetadata,
	runConfig *RunConfig,
	suppliedEnvVars []string,
	...
	if metadata != nil {
		...
		for _, envVar := range registryEnvVars {
			if isEnvVarProvided(envVar.Name, suppliedEnvVars, secretsList) {
				...
				//addNewVariable(ctx, envVar, envVar.Default, secretsManager, &envVars, &secretsList, debugMode)
...
func (c *CLIEnvVarValidator) Validate(
	ctx context.Context,
	metadata *registry.ImageMetadata,
	runConfig *RunConfig,
	suppliedEnvVars []string,
