...
// TestCreateRunConfigFromMCPRemoteProxy_AudienceOverrideWithEmbeddedAuthServer tests that when
...
func TestCreateRunConfigFromMCPRemoteProxy_AudienceOverrideWithEmbeddedAuthServer(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Add MCPOIDCConfig if the proxy references one
			if tt.proxy.Spec.OIDCConfigRef != nil {
				objects = append(objects, &mcpv1alpha1.MCPOIDCConfig{
					ObjectMeta: metav1.ObjectMeta{
						Name:      tt.proxy.Spec.OIDCConfigRef.Name,
						Namespace: tt.proxy.Namespace,
					},
