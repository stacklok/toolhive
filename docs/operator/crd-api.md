...
| `outputFormat` _string_ | OutputFormat is an optional Go template for constructing the workflow output.<br />If specified, the template is evaluated with access to:<br />  - .params.*          - Input parameters<br />  - .steps.*.output    - Step outputs<br />  - .steps.*.status    - Step status<br />  - .workflow.*        - Workflow metadata (id, duration, timestamps)<br />The template must produce valid JSON. If omitted, defaults to returning<br />the last step's output (backward compatible behavior).<br />Example:<br />  outputFormat: \|<br />    \{<br />      "data": \{<br />        "logs": \{\{.steps.fetch_logs.output\}\},<br />        "metrics": \{\{.steps.fetch_metrics.output\}\}<br />      \},<br />      "metadata": \{<br />        "workflow_id": "\{\{.workflow.id\}\}",<br />        "duration_ms": \{\{.workflow.duration_ms\}\}<br />      \}<br />    \} |  |  |
