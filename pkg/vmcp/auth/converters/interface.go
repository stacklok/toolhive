...
// to vMCP auth strategy configurations.
package converters
...
// StrategyConverter defines the interface for converting external auth configs to typed BackendAuthStrategy.
...
type StrategyConverter interface {
	...
	// Convert converts an MCPExternalAuthConfig to a typed BackendAuthStrategy.
	...
	Convert(externalAuth *mcpv1alpha1.MCPExternalAuthConfig) (*authtypes.BackendAuthStrategy, error)

	// ResolveSecrets fetches secrets from Kubernetes and replaces environment variable references
	// with actual secret values in the strategy config. This is used in discovered auth mode where
...
// ConvertToBackendAuthStrategy is a convenience function that uses the default registry to convert
// an external auth config to a typed BackendAuthStrategy. This is the main entry point for converting
// auth configs at runtime.
func ConvertToBackendAuthStrategy(
	externalAuth *mcpv1alpha1.MCPExternalAuthConfig,
) (*authtypes.BackendAuthStrategy, error) {
	if externalAuth == nil {
	...
	return converter.Convert(externalAuth)
}
...
// ExternalAuthConfigRef and resolves it to a typed BackendAuthStrategy.
// This is the main entry point for auth discovery from Kubernetes.
//
// Returns:
//   - strategy: The resolved BackendAuthStrategy with secrets fetched from Kubernetes
//   - error: Any error that occurred during discovery or resolution
//
// Returns nil if externalAuthConfigRef is nil (no auth configured).
func DiscoverAndResolveAuth(
	...
	// Convert to typed BackendAuthStrategy (without secrets resolved)
	strategy, err := converter.Convert(externalAuth)
	if err != nil {
