				...
				Config: vmcpconfig.Config{
					...
					Operational: &vmcpconfig.OperationalConfig{
						FailureHandling: &vmcpconfig.FailureHandlingConfig{
							// Use fast intervals for e2e testing
							// This allows the test to detect and report failures quickly
							HealthCheckInterval:     vmcpconfig.Duration(5 * time.Second),
							StatusReportingInterval: vmcpconfig.Duration(5 * time.Second), // Report status every 5s
							UnhealthyThreshold:      1,                                    // Mark unhealthy on first failure
						},
					},
				},
			},
