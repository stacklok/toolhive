	...
	tests := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T, strategy *vmcpconfig.BackendAuthStrategy) {
				...
				assert.Equal(t, "X-Upstream-Token", strategy.Metadata["external_token_header_name"])
			},
		},
		{
			...
			validate: func(t *testing.T, strategy *vmcpconfig.BackendAuthStrategy) {
				...
				assert.NotContains(t, strategy.Metadata, "client_id")
				assert.NotContains(t, strategy.Metadata, "client_secret_env")
				assert.NotContains(t, strategy.Metadata, "scopes")
				assert.NotContains(t, strategy.Metadata, "subject_token_type")
			},
		},
		{
			...
			validate: func(t *testing.T, strategy *vmcpconfig.BackendAuthStrategy) {
				t.Helper()
				assert.Equal(t, "urn:ietf:params:oauth:token-type:id_token", strategy.Metadata["subject_token_type"])
			},
		},
		{
...
func TestBuildOutgoingAuthConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "mixed mode with discovered and inline overrides",
			vmcp: &mcpv1alpha1.VirtualMCPServer{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-vmcp",
					Namespace: "default",
		...
		},
		{
			...
			validate: func(t *testing.T, config *vmcpconfig.OutgoingAuthConfig) {
				...
				assert.Contains(t, config.Backends, "backend-1")
				assert.Equal(t, "token_exchange", config.Backends["backend-1"].Type)
				assert.Equal(t, "https://oauth.example.com/token", config.Backends["backend-1"].Metadata["token_url"])
				assert.Equal(t, "test-client", config.Backends["backend-1"].Metadata["client_id"])
			},
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, strategy *vmcpconfig.BackendAuthStrategy) {
				...
				assert.Equal(t, "https://oauth.example.com/token", strategy.Metadata["token_url"])
				assert.Equal(t, "backend-service", strategy.Metadata["audience"])
				assert.Equal(t, "test-client", strategy.Metadata["client_id"])
			},
		},
		{
...
func TestDiscoverBackendsWithExternalAuthConfigIntegration(t *testing.T) {
	...
	assert.Equal(t, "https://oauth.example.com/token", authConfigResult.Backends["backend-1"].Metadata["token_url"])
	assert.Equal(t, "test-client-id", authConfigResult.Backends["backend-1"].Metadata["client_id"])
	assert.Equal(t, "backend-service", authConfigResult.Backends["backend-1"].Metadata["audience"])
